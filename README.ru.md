...
- **Таблица целевых бенчмарков (в стиле Anthropic)** — впишите жёсткие цели качества (общий балл, % улучшения, доля <UNK>, повторяемость, богатство словаря, доля настоящих слов); LLM-автопилот обязан их достичь и доказать цифрами — или честно сообщить о провале. Колонки «До обучения» и «Результат» заполняются автоматически по ходу обучения
- **Переносимый формат моделей** — скачанный `.pt` файл несёт в себе веса + конфиг + токенизатор; загрузите его на любой машине — и всё работает
- **Автопилот знает ваше железо** — агенту сразу сообщается CPU/GPU/RAM, и он подбирает размер модели под машину; детектор зависаний отличает медленную подготовку на слабом ПК от реального фриза
