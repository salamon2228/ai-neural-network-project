                ...
                <div class="grid" style="grid-template-columns:1fr 1fr;">
                    <div class="form-group">
                        ...
                        <select id="ap_provider" onchange="updateProviderFields()">
                            ...
                            <option value="ollama_cloud" data-i18n-option="autopilot_opt_ollama_cloud">Ollama Cloud (GPT-OSS, GLM, Qwen)</option>
...
var TRANSLATIONS = {
    ru: {
        ...
        autopilot_help_ollama_cloud: '<b>Ollama Cloud — большие облачные модели (GPT-OSS, GLM, Qwen):</b><br><br>1. Установите Ollama локально с <a href="https://ollama.com/download" target="_blank" style="color:#667eea;text-decoration:underline;">ollama.com/download</a> и войдите через <code>ollama signin</code> (или зарегистрируйтесь на сайте)<br>2. Откройте <a href="https://ollama.com/settings/keys" target="_blank" style="color:#667eea;text-decoration:underline;">ollama.com/settings/keys</a><br>3. Нажмите <b>«Create new key»</b> и скопируйте ключ<br>4. Вставьте его в поле <b>API ключ</b> выше<br>5. В поле <b>Модель</b> укажите название cloud-модели, например:<br>&nbsp;&nbsp;• <code>gpt-oss:120b</code> — открытая GPT-OSS 120B (рекомендуем для tool calling)<br>&nbsp;&nbsp;• <code>gpt-oss:20b</code> — поменьше и пошустрее<br>&nbsp;&nbsp;• <code>qwen3-coder:480b</code> — для кода<br>&nbsp;&nbsp;• <code>glm-4.6:cloud</code> — если включён доступ<br>&nbsp;&nbsp;• Список всех cloud-моделей: <a href="https://ollama.com/search?c=cloud" target="_blank" style="color:#667eea;text-decoration:underline;">ollama.com/search?c=cloud</a><br><br>💡 Cloud-модели запускаются на серверах Ollama — работают одинаково и с локального ПК, и с Google Colab. Удобно когда у Gemini кончились лимиты.',
    ...
    en: {
        ...
        autopilot_help_ollama_cloud: '<b>Ollama Cloud — large cloud models (GPT-OSS, GLM, Qwen):</b><br><br>1. Install Ollama locally from <a href="https://ollama.com/download" target="_blank" style="color:#667eea;text-decoration:underline;">ollama.com/download</a> and run <code>ollama signin</code> (or sign up on the website)<br>2. Open <a href="https://ollama.com/settings/keys" target="_blank" style="color:#667eea;text-decoration:underline;">ollama.com/settings/keys</a><br>3. Click <b>"Create new key"</b> and copy the key<br>4. Paste it into the <b>API Key</b> field above<br>5. In the <b>Model</b> field enter the cloud model name, for example:<br>&nbsp;&nbsp;• <code>gpt-oss:120b</code> — open GPT-OSS 120B (recommended for tool calling)<br>&nbsp;&nbsp;• <code>gpt-oss:20b</code> — smaller and faster<br>&nbsp;&nbsp;• <code>qwen3-coder:480b</code> — for code<br>&nbsp;&nbsp;• <code>glm-4.6:cloud</code> — if access is enabled<br>&nbsp;&nbsp;• Full list: <a href="https://ollama.com/search?c=cloud" target="_blank" style="color:#667eea;text-decoration:underline;">ollama.com/search?c=cloud</a><br><br>💡 Cloud models run on Ollama\'s servers — works the same from local PC and Google Colab. Handy when Gemini quota runs out.',
...
function updateProviderFields() {
    ...
    else if (p === 'ollama_cloud') m.placeholder = 'gpt-oss:120b';
...
function startAutopilot() {
    ...
    if (provider === 'google_ai') {
        ...
        sendProvider = 'openai_compatible';
        sendEndpoint = 'https://ollama.com/v1';
        if (!sendModel) sendModel = 'gpt-oss:120b';
    }
