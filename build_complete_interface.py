...
var TRANSLATIONS = {
    ru: {
        ...
        autopilot_help_google_colab: '<b>Бесплатная LLM через Google Colab:</b><br>1. Откройте <a href="https://colab.research.google.com/github/Slavikpro557/ai-neural-network-project/blob/main/colab_llm_server.ipynb" target="_blank" style="color:#667eea;text-decoration:underline;">ноутбук в Google Colab</a><br>2. Нажмите <b>Runtime → Run all</b><br>3. Зарегистрируйтесь на <a href="https://ngrok.com" target="_blank" style="color:#667eea;text-decoration:underline;">ngrok.com</a> (бесплатно) и вставьте токен в ноутбук<br>4. Скопируйте URL из вывода и вставьте в поле <b>Endpoint</b><br>5. Нажмите «Запустить автопилот»<br><br>💡 GPU Colab бесплатный, LLM работает на нём — ваш компьютер не нагружается.',
        autopilot_help_openai: '<b>OpenAI (платный):</b><br>1. Зарегистрируйтесь на <a href="https://platform.openai.com" target="_blank" style="color:#667eea;text-decoration:underline;">platform.openai.com</a><br>2. Перейдите в <b>API Keys → Create new secret key</b><br>3. Вставьте ключ (<code>sk-...</code>) в поле API ключ<br><br>💰 Стоимость: ~$0.01–0.05 за один запуск автопилота (GPT-4o).',
        autopilot_help_anthropic: '<b>Anthropic Claude (платный):</b><br>1. Зарегистрируйтесь на <a href="https://console.anthropic.com" target="_blank" style="color:#667eea;text-decoration:underline;">console.anthropic.com</a><br>2. Перейдите в <b>API Keys → Create Key</b><br>3. Вставьте ключ в поле API ключ<br><br>💰 Стоимость: ~$0.01–0.05 за один запуск автопилота.',
        autopilot_help_openai_compatible: '<b>Локальная LLM (бесплатно, но грузит ваш ПК):</b><br>1. Установите <a href="https://ollama.com" target="_blank" style="color:#667eea;text-decoration:underline;">Ollama</a> или <a href="https://lmstudio.ai" target="_blank" style="color:#667eea;text-decoration:underline;">LM Studio</a><br>2. Скачайте модель: <code>ollama pull llama3</code><br>3. Запустите сервер (Ollama запускается автоматически)<br>4. Endpoint по умолчанию: <code>http://localhost:11434/v1</code><br><br>⚠️ Требует мощный ПК (16GB+ RAM). LLM работает одновременно с обучением.',
    ...
    en: {
        ...
        autopilot_help_google_colab: '<b>Free LLM via Google Colab:</b><br>1. Open the <a href="https://colab.research.google.com/github/Slavikpro557/ai-neural-network-project/blob/main/colab_llm_server.ipynb" target="_blank" style="color:#667eea;text-decoration:underline;">notebook in Google Colab</a><br>2. Click <b>Runtime → Run all</b><br>3. Sign up at <a href="https://ngrok.com" target="_blank" style="color:#667eea;text-decoration:underline;">ngrok.com</a> (free) and paste the token into the notebook<br>4. Copy the URL from the output and paste it into the <b>Endpoint</b> field<br>5. Click "Start Autopilot"<br><br>💡 Colab GPU is free, the LLM runs there — your computer stays unloaded.',
        autopilot_help_openai: '<b>OpenAI (paid):</b><br>1. Sign up at <a href="https://platform.openai.com" target="_blank" style="color:#667eea;text-decoration:underline;">platform.openai.com</a><br>2. Go to <b>API Keys → Create new secret key</b><br>3. Paste the key (<code>sk-...</code>) into the API Key field<br><br>💰 Cost: ~$0.01–0.05 per autopilot run (GPT-4o).',
        autopilot_help_anthropic: '<b>Anthropic Claude (paid):</b><br>1. Sign up at <a href="https://console.anthropic.com" target="_blank" style="color:#667eea;text-decoration:underline;">console.anthropic.com</a><br>2. Go to <b>API Keys → Create Key</b><br>3. Paste the key into the API Key field<br><br>💰 Cost: ~$0.01–0.05 per autopilot run.',
        autopilot_help_openai_compatible: '<b>Local LLM (free, but uses your PC resources):</b><br>1. Install <a href="https://ollama.com" target="_blank" style="color:#667eea;text-decoration:underline;">Ollama</a> or <a href="https://lmstudio.ai" target="_blank" style="color:#667eea;text-decoration:underline;">LM Studio</a><br>2. Download a model: <code>ollama pull llama3</code><br>3. Start the server (Ollama starts automatically)<br>4. Default endpoint: <code>http://localhost:11434/v1</code><br><br>⚠️ Requires a powerful PC (16GB+ RAM). LLM runs alongside model training.',
...
function updateProviderFields() {
    ...
    document.getElementById('ap_api_key_group').style.display = (p === 'google_colab') ? 'none' : 'block';
